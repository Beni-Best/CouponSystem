...
public class CompanyFacade implements CouponClientFacade {
	...
	public void removeCouponById(Coupon coupon) throws CouponSystemException {

		Coupon tmpCoup = new Coupon();

		tmpCoup = coupDao.getCoupon(coupon.getId());
		if (tmpCoup == null) {
			throw new CouponSystemException("There is no such coupon to remove");
		}
		if (joinDao.checkCompanyCouponConnect(company, coupDao.getCoupon(tmpCoup.getTitle())) == false) {
			throw new CouponSystemException("The coupon you wanna remove not belong to your company or not exist");
		}

		joinDao.deleteCoupon(coupDao.getCoupon(tmpCoup.getTitle()));
		coupDao.removeCoupon(coupDao.getCoupon(tmpCoup.getTitle()));
	...
	public void updateCoupon(Coupon coupon) throws CouponSystemException {
		Coupon tmpCoup = new Coupon();
		System.out.println("coupon with function we get is "+coupon);
		tmpCoup = coupDao.getCoupon(coupon.getTitle());
		...
		if (tmpCoup.getTitle().equals(coupon.getTitle()) && tmpCoup.getImage().equals(coupon.getImage())
				...
				&& tmpCoup.getStart_Date().toString().equals(coupon.getStart_Date().toString()) && tmpCoup.getType().equals(coupon.getType())
...
//
//		// check if not updating something except end-Date and Price
//		if (tmpCoup.getStart_Date().toString().equals(coupon.getStart_Date().toString())) {
